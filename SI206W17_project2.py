...
## INPUT: N/A. No input.
## The function should check if there is any cached data for the UMSI directory in your cached file -- if so, return it, 
## and if not, the function should access each page of the directory, get the HTML associated with it, append that HTML string to a list. 
...
def get_five_tweets(word):
	...
	if twitter_identifier in CACHE_DICTION:
		...
		f = codecs.open(CACHE_FILE,'w', encoding="utf-8") # codecs.open the cache file for writing, using the third parameter by following Yu-Jen's piazza post
		f.write(json.dumps(CACHE_DICTION)) # dumps function makes the whole dictionary holding data and unique identifiers into a json-formatted string
		f.close()
		#desired_tweets = CACHE_DICTION["statuses"]
...
for i in range(5):
	x = find_urls(five_tweets[i]) #do it one by one
	for y in x: 
		tweet_urls_found.append(y) #appends strings!!!! finally solved it
